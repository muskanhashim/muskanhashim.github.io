...
Single-cell analyses built on scverse tools can be the starting point for biological interpretation. For example, after clustering with [Scanpy](https://scanpy.readthedocs.io/) and identifying marker genes, a researcher typically wants to know what those genes do, how they are expressed across tissues, whether they are associated with disease, and how their protein products are structured. Without tooling, answering these questions means leaving Python and visiting several web portals.

gget closes that loop and is designed for exactly the questions that arise at the end of a scverse analysis. It naturally returns results in formats that fit back into the analysis environment.
...
The `gget.cellxgene` module returns data directly as [AnnData](https://anndata.readthedocs.io/) objects helping slot results immediately into scverse pipelines:
...
A researcher studying spatial transcriptomics with Squidpy may want to fetch ligand-receptor protein structures from PDB, or pull disease associations from Open Targets, or BLAST a sequence of interest. The integration of gget makes each of those a one-liner by being the layer that helps you handle external reference databases alongside those analyses.
...
For gget, joining scverse means adopting shared community infrastructure: the [scverse package template](https://github.com/scverse/cookiecutter-scverse), continuous integration conventions, and the documentation and testing practices the ecosystem shares. It also means participating in the same community spaces where the broader ecosystem is discussed and developed.
...
*gget is an open-source project available under the Apache 2.0 license. Contributions are welcome via the [GitHub repository](https://github.com/scverse/gget).*
